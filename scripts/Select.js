...
import MatchMedia from "./generic/MatchMedia.js"
...
class Select extends BaseComponent {
    ...
    onClick = (event) => {
        const { target } = event
        const isOutsideDropdownCliick = target.closest(this.selectors.dropdown) !== this.dropdownElement
        const isButtonClick = target === this.buttonElement

        if(!isButtonClick && isOutsideDropdownCliick) {
            this.collapse()
            return
        }
    ...
    originalControlChange = () => {
        this.state.selectedOptionElement = this.originalControlElement[this.originalControlElement.selectedIndex]
    }
